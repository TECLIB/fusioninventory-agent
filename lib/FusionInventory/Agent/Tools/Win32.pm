...
sub _getRegistryKeyFromWMI{
    ...
    my $keys = Win32::OLE::Variant->new(Win32::OLE::Variant::VT_BYREF() | Win32::OLE::Variant::VT_BSTR() | Win32::OLE::Variant::VT_ARRAY(), 128);
    my $return = $objReg->EnumKey($hkey, $params{keyName}, $keys);
#    my @dim = $keys->Dim;
#    my $subKeys = \@dim;
    my $subKeys = [];
    push @$subKeys, $return;
    push @$subKeys, $keys->Get(1, 0);
    push @$subKeys, $keys->Get(2, 0);
    push @$subKeys, $keys->Get(3, 0);
